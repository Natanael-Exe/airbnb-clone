...
const GuestModal = ({
  ...
  return (
    <div
      className="rounded-md bg-white border shadow-lg p-4 flex flex-col absolute inset-x-0 space-y-6  w-full transiton duration-200 ease-in"
      onDoubleClick={(e) => {
        e.preventDefault();
        e.stopPropagation();
      }}
    >
      <div className="flex items-center justify-between">
        ...
        <p className="inline-flex items-center ">
          <MinusSmIcon
            ...
            className={`rounded-full w-9 h-9 flex-shrink-0 mr-2 p-2 border ${
              adults == 1
                ? "text-gray-300 cursor-not-allowed"
                : "hover:border-gray-600 text-gray-600"
            } cursor-pointer hover:border-gray-600 preventDoubleClick`}
          />
          {adults}
          <PlusSmIcon
            ...
            className={`rounded-full w-9 h-9 flex-shrink-0 ${
              adults == 2
                ? "text-gray-300 cursor-not-allowed"
                : "hover:border-gray-600 text-gray-600"
            }  p-2 border ml-2 cursor-pointer preventDoubleClick`}
          />
        </p>
      </div>
      ...
      <p className="text-sm font-light w-10/12 text-left">
        2 guests maximum. Infants don’t count toward the number of guests.
      </p>
      <p
        className="text-center ml-auto font-semibold underline cursor-pointer p-2 hover:bg-gray-100 hover:rounded-md inline w-16"
